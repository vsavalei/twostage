...
```{r comp_vs_ts2}
max(abs(comp_table$est.x - comp_table$est.y)) 
max(abs(comp_table$se - comp_table$se_ts)) 
```
...
Instead, it will be equal to the chi-square from the GLS method, which can be verified by running:
...
```{r PIM3}
comp_table <- compare_est(fit_comp,fit_pim)[,c("est.x","se.x","est.y","se.y")]
max(abs(comp_table$est.x - comp_table$est.y)) 
max(abs(comp_table$se.x - comp_table$se.y)) 
```
