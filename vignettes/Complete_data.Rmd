...
We then specify the composite-level model of full mediation ($df=5$) and fit it in `lavaan`: 
...
```
The printed chi-square, df, parameter estimates, and standard errors from `fit_comp` will be the values to match by the item-level methods for fitting composite models.

## Item-level Methods for Composites

These methods fit the same model to the original dataset containing only the items, without computing the composites explicitly. For all approaches, we first need to define the matrix $C$ that relates composites to components. The end user can assign components to composites interactively as follows:
...
```{r TSML2}
#summary(fit_tsml) #does not work, conflict with summary from lavaan?
#class(fit_tsml) #only twostage
#class(fit_tsml$TS_Run_naive) #this is of class lavaan
summary.twostage(fit_tsml) #does work
```
...
### Approximate Fit Assessment (separate vignette eventually)

This is more complicated. With complete data, fit measures for the direct composites run and the naive Stage 2 fit measures for the TSML run are the same: 
...
With incomplete data, even "naive" TSML estimates of RMSEA, CFI, etc will be better, as they will be based on the EM covariance matrix and means obtained in Stage 1, whereas a direct composites run would have to use some kind of available case analysis. However, small sample corrections to TSML fit indices could still be implemented to improve their performance, as discussed in [Zhang and Savalei (2023)](https://psycnet.apa.org/doiLanding?doi=10.1037%2Fmet0000445), though this article did not consider item-level TS when the model is at the composite level. (This is probably a paper)
