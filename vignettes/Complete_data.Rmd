...
Estimates and standard errors from `fit_comp` and `fit_ts` are also identical to six decimal places. Below is their comparison side by side (rounded to three decimal places):  
...
With complete data, fit measures from the model fit directly to the composites (`fit_comp`) and the "naive" fit measures from Stage 2 of TSML (`fit_ts`) are the same. (This will not be the case with incomplete data). On the other hand, some PIM fit measures are weird:  
...
Specifically, while the PIM chi-square test and the RMSEA are unaffected, the CFI and other relative fit index values is different. A custom baseline model would have to be specified to get the right CFI with PIM. The PIM SRMR is also different, and adjustments are needed to exclude the items.

While the PIM AIC/BIC values are different, they are not relevant and should not be compared across different composite-model fitting methods. They *can* be compared for different composite models within the same type of method -- e.g., different models all fit via PIM. 
