...
Indices depending on the baseline model (e.g., CFI) and those depending on the residuals (e.g., SRMR), as well as information criteria (e.g., AIC) are different for PIMs. Some of these can be recomputed using a different baseline model for a better evaluation of the fit of just the composite model in the PIM setup; see [Approximate_fit vignette](../doc/Approximate_fit.html).
