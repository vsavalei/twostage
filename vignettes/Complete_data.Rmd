...
#Use devtools::build_rmd("vignettes/Complete_data.Rmd") to just test one vignette

#To build all vignettes properly, run devtools::build_vignettes(), not build_rmd above. This command #ensures all vignettes are compiled correctly and links are processed within the package context.
```
...
```{r TSML1}
fit_ts <- twostage(data = tpbdata, model = tpbmod, C = C,
...
In this case, because the data are complete and we have matched the type of information used in Stage 1 to what is used by default with complete data, the "naive" and "TSML" standard errors are identical. 

In addition, estimates and standard errors from `fit_comp` and `fit_ts` are identical to six decimal places. Below is their comparison side by side (rounded to three decimal places):  
...
Here are the maximum differences:

```{r comp_vs_ts2}
max(abs(comp_table$est.fit_comp - comp_table$est.fit_ts)) 
max(abs(comp_table$se.fit_comp - comp_table$se.fit_ts)) 
```
...
```{r comp_vs_ts4}
chisq_ts <- fit_ts@Fit@test$twostage$test
df <- fit_ts@Fit@test$twostage$df
pvalue_ts <- fit_ts@Fit@test$twostage$pval
print(c(chisq_ts,df,pvalue_ts))
```

Because it is computed as a quadratic form, and does not use the ML fit function equation, it will be highly similar, but not numerically identical to the "naive" ML chi-square even when the data are complete. (Note: In the original references, $N-1$ was used as a sample size multiplier, but $N$ is used in the `twostage` code for consistency with `lavaan` defaults for all other statistics.)

<!-- commented out GLS because it looks like the GLS chi-square is N*2*fmin, yet it matches TS with N-1, not N. No idea what's going on. 

Instead, it will be closer to the chi-square from the GLS method (differences are likely due to using $N$ versus $N-1$: 

```{r gls}
fit_comp_gls<-lavaan::sem(tpbmod,data=tpbdatac,fixed.x=FALSE,
                          meanstructure=TRUE,estimator="GLS")
lavaan::fitmeasures(fit_comp_gls)[c("chisq","df","pvalue")] 
...
max(abs(comp_table$est.fit_comp - comp_table$est.fit_pim)) 
max(abs(comp_table$se.fit_comp - comp_table$se.fit_pim)) 
```
<!--I wonder what the differences in the 6th decimal place are due to!-->

As a sidenote, we could have also matched the standard errors for all three approaches by using observed information, by specifying ```information='observed'``` for `fit_comp` and `fit_pim` runs, and omitting `runcommand="information='expected'"` for `fit_ts`.  
...
On the other hand, some PIM fit metrics can be weird: 
```{r afi2}
afi_pim <- lavaan::fitmeasures(fit_pim)
round(cbind(afi_comp,afi_ts_naive,afi_pim),3)
```
Specifically, while the chi-square test and the RMSEA are unaffected, a custom baseline model would have to be specified to get the right CFI and other relative fit indices with PIM. The SRMR also needs to be computed differently. The AIC/BIC values are not relevant and should not be compared for the same composite model across different composite-model fitting methods. (But they can be compared for different models within the same type of method -- e.g., different models all fit via PIM or via TS). 
