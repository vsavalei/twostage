...
compmodel<-"C1 ~ C2 + C3"
model1 <- PIM_syntax(compmodel = compmodel, C = C)
cat(model1)
data1 <- misdata_mcar20[,c("Y1","Y2","Y3","Y4","Y5","Y6","Y7")]
out<-lavaan::lavaan(model = model1,data = data1)
df <- as.numeric(lavaan::fitmeasures(out)["df"])
df

#made up example renaming C3 into Y7:
C2 <- C
rownames(C2)<-c("C1","C2","Y7")
compmodel2<-"C1 ~ C2 + Y7"
model2 <- PIM_syntax(compmodel = compmodel2, C = C2)
cat(model2)
out2 <- lavaan::lavaan(model2,data=data1) 
...
out2.mod <- lavaan::lavaan(model2.mod,data=data1)
out2.mod
#the default is to correlate predictors when one of the predictors is observed
#this is reported on github, at Yves' request #414

```

<!-- Note to self: I just realized that this function has no option for excluding
...
be assigned to their own `composite`. But the user may feed only a subportion 
of the data that truly are components: we need to have informative messages 
to guide all this.-->   
