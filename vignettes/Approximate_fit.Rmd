...
To get the right CFI/TLI, we need a new baseline model that only un-correlates variables in the composite model, while leaving the rest of the PIM structure intact. The function `compmodel_base` will create a baseline model, which un-correlates all the variables (except possibly for exogenous predictors), while estimating their means and variances. We recommend to always look at the baseline model syntax to verify it is what was intended (and modify as needed).
...
#tpbpim_base <- PIM_syntax(C=C,compmodel=basemod,exog_cov=FALSE)
```

The only difference between the function `PIM_syntax_base` and the original function `PIM_syntax` is that the former does not have `exog_cov=TRUE` argument, creating syntax that is equivalent to setting it to false. The baseline model syntax can be obtained equivalently using the original function as shown above (commented out). Because in the baseline model all variables are exogenous, setting `exog_cov=TRUE` would correlate *all* of them, resulting in a saturated composites model, not a baseline composites model. Thus, if using the original function `PIM_syntax`, we must set it to false, and explicitly add the correlations among the variables that are exogenous in the H0 model `tpbmod` via the syntax created by `compmodel_base`. This will be fixed in the future to be less confusing!! 
...
#summary(fit_pim,fit.measures=TRUE) #slow b/c robust
#SRMR = 0.024
#Robust RMSEA = .024, RMSEA =.008 
# CFI=.999, TLI = .981, Robust CFI = .990, Robust TLI = .861

fitm_pim <- lavaan::fitMeasures(fit_pim)
round(fitm_pim[c("cfi","cfi.robust","tli","tli.robust","rmsea","rmsea.robust","srmr_mplus","srmr_mplus_nomean")],3) #slow b/c robust, figure out why (diff project)
#source: lav_fit_fiml_corrected function in lav_fit_utils.R
#
#below is how to toggle robust measures to off to speed things up:
#fitm_pim_norobust <- lavaan::fitMeasures(fit_pim,fm.args = list(robust = FALSE))
#fitm_pim_norobust["cfi.robust"]
```
...
```{r}
basemod <- compmodel_base(mod)
cat(basemod)

mod_base <- PIM_syntax_base(C=C,compmodel=basemod)
...
```{r}
pim_sat <- PIM_syntax_sat(C=C,compmodel=mod)
fit_pim_sat <- lavaan::lavaan(pim_sat, data=misdata_mcar20)

srmr <- srmr_mplus_pim(fit_pim,fit_pim_sat,mod)
srmr
```
...
## Obtaining Better Small Sample Properties for TSML Fit Indices with Incomplete Data

When it comes to TSML-based RMSEA and CFI/TLI, unlike their FIML counter-parts, their asymptotic values are already correct and do not require adjustments. However, they can still receive a small sample adjustment that basically amounts to subtracting more than just df to get the noncentrality parameters. The indices would also not to be based on $T_\text{res}$ with normal data, but on the original chi-square $T$, with an appropriate $df$ subtraction. I need to work this out? 
